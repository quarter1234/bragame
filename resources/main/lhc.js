...
/*		var data = {
        ...
        var yopentime = yopentimes_arr[1];
				html += '<div class="lhczs8899"><div class="lhcqs8899" width="130">'+openinfo.expect+'</div><div class="lhcqh8899"><span width="46" class="c_red2 fb">'+parseInt(array[0])+'</span><span width="46" class="c_red2 fb">'+parseInt(array[1])+'</span><span width="46" class="c_red2 fb" >'+parseInt(array[2])+'</span><span width="46" class="c_red2 fb">'+parseInt(array[3])+'</span><span width="46" class="c_red2 fb">'+parseInt(array[4])+'</span><span width="46" class="c_red2 fb">'+parseInt(array[5])+'</span><span width="46" class="c_red99 fb" >+</span><span width="46" class="c_red2 fb" >'+parseInt(array[6])+'</span></div><span class="" width="40" class="c_blue fb " style="width: 18%;text-align: center;float: left;min-height: 30px;line-height: 40px;word-wrap: break-word;">'+yopentime+'</span></div>';
            ...
            if (msg.sign == true) {
                if (msg.data.opencode.length > 0) {
                    ...
                    if (lastExpect == openexpect) {
                        ...
                        var yopentimes_arr = yopentimes.split(' ');
                        var yopentime = yopentimes_arr[1];
                        var html = '<div class="lhczs8899"><div class="lhcqs8899" width="130">' + openexpect + '</div><div class="lhcqh8899';
                        html += '">';
                        for (var j = 0; j < openCodes.length; j++) {
                          if(j == openCodes.length - 1){
                            html += '<span width="46" class="c_red99 fb" >+</span><span width="46" class="c_red2 fb">' + openCodes[j] + '</span>';
                          } else {
                            html += '<span width="46" class="c_red2 fb">' + openCodes[j] + '</span>';
                          }
                        }
                        html += '</div><span class="" width="40" class="c_blue fb " style="width: 18%;text-align: center;float: left;min-height: 30px;line-height: 40px;word-wrap: break-word;">'+yopentime+'</span></div>';
...
function setLotterNumber(_index,_nums) {
  ...
  if(_nums == 'gif'){
    ...
    $('#lhc_winning_sum').find('li').css('text-indent','-9999px');
		...
		console.log('lhc_numData.lhc_'+_nums);
