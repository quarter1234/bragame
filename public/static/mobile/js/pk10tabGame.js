$(function () {
    var inputVal = ''; //用户填写的倍数 selectNumbers
    var zhushus = []; //注数数组;
...
console.log(JSON.stringify(Numbers));
    ...
    function gameNumber(arr, number, start) {
        // yrates[playid].rate
        var dwd_rate = yrates['bjpk10dwd']['maxjj'];
        ...
        for (var i = 0; i < arr.length; i++) {
            ...
            if (number && start) {
                for (var j = start; j <= number; j++) {
                    if (j < 10) {
                        boxNumber.append('<a href="javascript:void(0);" class="selectNumber" data-number="' + j + '"><span>0'+j+'</span><span style="display:block;color:#999;font-size:12px;font-weight: normal;">'+dwd_rate+'</span></a>');
                    } else {
                        boxNumber.append('<a href="javascript:void(0);" class="selectNumber" data-number="' + j + '"><span>'+j+'</span><span style="display:block;color:#999;font-size:12px;font-weight: normal;">'+dwd_rate+'</span></a>');
                    }
                }
            } else if (number) {
                for (var j = 0; j <= number; j++) {
                    if (j < 10) {
                        boxNumber.append('<a href="javascript:void(0);" class="selectNumber" data-number="0' + j + '"><span>0'+j+'</span><span style="display:block;color:#999;font-size:12px;font-weight: normal;">'+dwd_rate+'</span></a>');
                    } else {
                        boxNumber.append('<a href="javascript:void(0);" class="selectNumber" data-number="' + j + '"><span>'+j+'</span><span style="display:block;color:#999;font-size:12px;font-weight: normal;">'+dwd_rate+'</span></a>');
                    }
                }
    ...
    function currList() {
        ...
        $('.selectNumbers').each(function (i) {
            ...
            $(this).find('.curr').each(function (i) {
                acArr.push($(this).find('span:first-child').text());
            })
            currArr.push(acArr);
        })
        return currArr;
    ...
    $('.betting_right_btn').on('click',function(){
     ...
     //点击投注按钮 
    $('.betting_right_btn2').on('click',function(){
        $('.addtobetbtn').click();
        $("#f_submit_order").click();
    });
